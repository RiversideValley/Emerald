...
public partial class Program
{
    ...
    {
        api.MapGet("/status", (Core c, INetworkCapabilityService network, IDownloadActivityService downloads) =>
                Results.Ok(new
                {
        ...
        api.MapPost("/core/initialize",
                async (InitializeCoreRequest req, Core c, IDownloadActivityService downloads,
                    ILogger<Program> logger) =>
                {
                    ...
                    if (downloads.Snapshot.ActiveDownloads > 0)
                    {
                        return Results.Conflict(new
                            { Error = "Minecraft path cannot be changed while downloads are active." });
                    }
        ...
        api.MapGet("/accounts/selected", (IAccountService ac) =>
            {
                ...
                if (selected == null)
                {
                    return Results.NotFound(new { Error = "No account is currently selected." });
                }
        ...
        api.MapPost("/accounts/select", (AccountSelectionRequest req, IAccountService ac) =>
            {
                ...
                if (account == null)
                {
                    return Results.NotFound(new { Error = $"Account with identifier '{req.Identifier}' not found." });
                }
        ...
        api.MapPost("/accounts/offline", async (OfflineAccountRequest req, IAccountService ac) =>
            {
                ...
                await ac.SignInAsync(AccountProviderIds.Offline,
                    new AccountSignInRequest(method.MethodId, req.Username));
                var selected = ac.GetSelectedAccount();
                return Results.Ok(new
                {
                    Message = "Offline account created and selected.", Username = selected?.Name,
                    Identifier = selected?.UniqueId
                });
            })
        ...
        api.MapDelete("/accounts/{identifier}", async (string identifier, IAccountService ac) =>
            {
                ...
                if (account == null)
                {
                    return Results.NotFound(new { Error = $"Account '{identifier}' not found." });
                }
        ...
        api.MapGet("/accounts/login/microsoft/status", async (IAccountService ac) =>
            {
                if (_loginTask == null)
                {
                    return Results.NotFound(new { Status = "no_login_in_progress" });
                }
                ...
                if (_loginTask.IsFaulted)
                {
                    return Results.BadRequest(new
                        { Status = "failed", Error = _loginTask.Exception?.InnerException?.Message });
                }

                var selected = ac.GetSelectedAccount();
                return Results.Ok(new
                    { Status = "completed", Username = selected?.Name, Identifier = selected?.UniqueId });
            })
        ...
        AccountSignInInputKind inputKind)
    {
        ...
        return provider.SignInMethods.FirstOrDefault(candidate => candidate.InputKind == inputKind)
               ?? throw new InvalidOperationException(
                   $"Account provider '{providerId}' has no compatible sign-in method.");
    }
        ...
        api.MapPost("/games", (CreateGameRequest req, Core c) =>
            {
                ...
                var loaderType = Enum.TryParse<Emerald.CoreX.Versions.Type>(req.LoaderType, true, out var parsed)
                    ? parsed
                    : CoreX.Versions.Type.Vanilla;
        ...
        api.MapDelete("/games", (string basePath, bool deleteFolder, Core c) =>
            {
                ...
                if (game == null)
                {
                    return Results.NotFound(new { Error = $"Game instance at path '{basePath}' not found." });
                }
    ...
    {
        api.MapPost("/games/launch", async (GameLaunchRequest req, IGameRuntimeService runtime,
                IInstanceInstallationService installation, INetworkCapabilityService network, Core c,
                IAccountService ac, ILogger<Program> logger) =>
            {
                var game = c.Games.FirstOrDefault(g => g.Path.BasePath == req.BasePath);
                if (game == null)
                {
                    return Results.NotFound(new { Error = $"Game instance at path '{req.BasePath}' not found." });
                }

                var readiness = await installation.PrepareLaunchAsync(game);
                if (!readiness.CanLaunch)
                {
                    return Results.BadRequest(readiness);
                }
                ...
                {
                    return Results.BadRequest(new
                        { Error = "No account selected. Please select or create an account first." });
                }

                if (RequiresOfflineAccount(network, ac, account))
                {
                    return OfflineAccountRequiredResult();
                }
    ...
    private static bool RequiresOfflineAccount(
        ...
        EAccount account)
    {
        return network.GetSnapshot(NetworkCapability.MinecraftMetadata).EffectiveState ==
    ...
    {
        return Results.BadRequest(new { Error = "Emerald is offline. Select an offline account before launching." });
    }

    private static async Task LaunchInBackgroundAsync(Game game, EAccount account, IGameRuntimeService runtime,
        ILogger<Program> logger)
    {
        try
        {
            logger.LogInformation("Launching game instance: {Name} ({Path})", game.Version.DisplayName,
                game.Path.BasePath);
    ...
    private static void MapVersionRoutes(RouteGroupBuilder api)
    {
        ...
        api.MapGet("/versions/loaders", async (
            ...
            {
                if (!Enum.TryParse<Emerald.CoreX.Versions.Type>(loaderType, true, out var type)
    ...
    private static void MapSettingsRoutes(RouteGroupBuilder api)
    {
        ...
        api.MapPut("/settings/shared-store",
                (SharedStoreSettingsRequest req, IStoreSharedContentSettingsService settingsService) =>
                {
                    ...
                    return Results.Ok(new
                    {
                        Message = "Shared store settings updated successfully.", Settings = settingsService.Settings
                    });
                })
