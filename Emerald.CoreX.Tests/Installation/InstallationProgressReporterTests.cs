...
public sealed class InstallationProgressReporterTests
{
    ...
    public async Task Reporter_CoalescesHighFrequencyUpdates_AndFlushesFinalState()
    {
        ...
        {
            reporter.Report(new InstallationProgress("Downloading", $"file-{index}", index, 8_000));
        }

        await reporter.CompleteAsync(new InstallationProgress("Complete", "done", 8_000, 8_000), true, "Complete");
    ...
    public async Task Reporter_CompletesNotificationOnlyOnce()
    {
        ...
        await reporter.CompleteAsync(new InstallationProgress("Complete", "done", 1, 1), true, "Complete");
        await reporter.CompleteAsync(new InstallationProgress("Failed", "done", 1, 1), false, "Should be ignored");
