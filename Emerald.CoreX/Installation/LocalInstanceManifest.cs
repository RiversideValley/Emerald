...
internal static class LocalInstanceManifest
{
    ...
    private static async Task<string?> AddVersionFilesAsync(Game game, string version,
        Dictionary<string, ExpectedManagedFile> files,
        CancellationToken cancellationToken)
    {
        ...
        Add(files, new ExpectedManagedFile(ManagedPathRoot.Versions, jsonRelative, FileSize(jsonPath),
            FileSha1(jsonPath), null,
    ...
    private static void AddClientFile(JsonElement root, string version, Dictionary<string, ExpectedManagedFile> files)
    {
        ...
        Add(files,
            FromDownload(ManagedPathRoot.Versions, relative, client, ManagedFileCategory.Client,
    ...
    private static void AddLibraries(JsonElement root, Dictionary<string, ExpectedManagedFile> files)
    {
        ...
        {
            AddLibrary(library, files);
        }
    }

    private static void AddLibrary(JsonElement library, Dictionary<string, ExpectedManagedFile> files)
    {
        if (!IsAllowedOnCurrentPlatform(library) || !library.TryGetProperty("downloads", out var downloads))
        {
            return;
        }
    ...
    {
        if (!downloads.TryGetProperty("artifact", out var artifact) || !artifact.TryGetProperty("path", out var path))
        {
            return;
        }
    ...
    private static void AddNativeClassifiers(JsonElement downloads, Dictionary<string, ExpectedManagedFile> files)
    {
        ...
        {
            if (!IsCurrentNativeClassifier(classifier.Name) || !classifier.Value.TryGetProperty("path", out var path))
            {
                continue;
            }
    ...
    private static void AddAssetIndex(Game game, JsonElement root, Dictionary<string, ExpectedManagedFile> files,
        CancellationToken cancellationToken)
    {
        if (!root.TryGetProperty("assetIndex", out var assetIndex) || !assetIndex.TryGetProperty("id", out var assetId))
        {
            return;
        }
        ...
        Add(files,
            FromDownload(ManagedPathRoot.Assets, relative, assetIndex, ManagedFileCategory.Metadata,
                IntegritySeverity.Critical));
    ...
    private static void AddLoggingFile(JsonElement root, Dictionary<string, ExpectedManagedFile> files)
    {
        ...
        Add(files, FromDownload(ManagedPathRoot.Assets, SafeRelative(Path.Combine("log_configs", id.GetString()!)),
            file,
    ...
    public static string ComputePathFingerprint(Game game)
    {
        ...
        var value = string.Join('\n', game.Path.BasePath, game.Path.Assets, game.Path.Library, game.Path.Runtime,
            game.Path.Versions);
        return Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(value))).ToLowerInvariant();
    }

    private static void AddAssetObjects(Game game, Dictionary<string, ExpectedManagedFile> files, string indexRelative,
        CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!item.Value.TryGetProperty("hash", out var hashElement))
            {
                continue;
            }
            ...
            Add(files, new ExpectedManagedFile(ManagedPathRoot.Assets, relative, GetLong(item.Value, "size"), hash,
                null,
    ...
    private static ExpectedManagedFile FromDownload(ManagedPathRoot root, string relative, JsonElement element,
        ManagedFileCategory category, IntegritySeverity severity)
    {
        return new ExpectedManagedFile(root, relative, GetLong(element, "size"),
    ...
    private static bool IsAllowedOnCurrentPlatform(JsonElement library)
    {
        ...
        foreach (var rule in rules.EnumerateArray())
        {
            ...
            if (matches && rule.TryGetProperty("action", out var action))
            {
                allowed = action.GetString() == "allow";
            }
        }
    ...
    private static void Add(Dictionary<string, ExpectedManagedFile> files, ExpectedManagedFile file)
    {
        files[$"{file.Root}:{file.RelativePath}"] = file;
    }
