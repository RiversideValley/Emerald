...
public enum PlaytimeScopeKind { AllEmerald, CurrentBase, Instance }
public enum PlaytimeRange { SevenDays, ThirtyDays, NinetyDays, AllTime }
...
public sealed record PlaytimeScope(PlaytimeScopeKind Kind, string? BasePath = null, Guid? InstanceId = null)
{
    ...
    public static PlaytimeScope ForBase(string basePath) => new(PlaytimeScopeKind.CurrentBase, Normalize(basePath));
    public static PlaytimeScope ForInstance(string basePath, Guid instanceId) => new(PlaytimeScopeKind.Instance, Normalize(basePath), instanceId);
...
public sealed record InstancePlaytimeRanking(Guid InstanceId, string BasePath, string DisplayName, TimeSpan Duration, int SessionCount);
...
{
    event EventHandler? HistoryChanged { add { } remove { } }
...
public sealed class InstancePlaytimeService : IInstancePlaytimeService
{
    ...
    public IReadOnlyList<InstancePlaytimeSession> GetSessions(string instancePath)
    {
        ...
        lock (_syncRoot)
            return ReadHistory().Where(x => PathsEqual(x.InstancePath, normalizedPath)).OrderByDescending(x => x.StartedAt).ToArray();
    }

    public TimeSpan GetTotalPlaytime(string instancePath)
        => GetSessions(instancePath).Aggregate(TimeSpan.Zero, (total, session) => total + session.Playtime);
    ...
    public IReadOnlyList<InstancePlaytimeSession> GetSessions(PlaytimeScope scope)
    {
        lock (_syncRoot)
        {
            ...
            if (scope.Kind is PlaytimeScopeKind.CurrentBase or PlaytimeScopeKind.Instance)
                query = query.Where(x => !string.IsNullOrWhiteSpace(scope.BasePath) && PathsEqual(x.BasePathSnapshot, scope.BasePath));
    ...
    public void RecordSession(InstancePlaytimeSession session)
    {
        ...
        if (session.EndedAt < session.StartedAt) throw new ArgumentException("A playtime session cannot end before it starts.", nameof(session));
        session.InstancePath = NormalizePath(session.InstancePath);
        if (!string.IsNullOrWhiteSpace(session.BasePathSnapshot)) session.BasePathSnapshot = NormalizePath(session.BasePathSnapshot);
        lock (_syncRoot)
        {
            var history = ReadEnvelope();
            if (_isReadOnly) return;
            if (history.Sessions.Any(existing => existing.Id == session.Id)) return;
            history.Sessions.Add(session);
            _baseSettingsService.Set(SettingsKeys.PlaytimeHistory, history);
        }
        _logger.LogInformation("Recorded {Playtime} of playtime for instance {InstanceId}.", session.Playtime, session.InstanceId);
    ...
    public PlaytimeAnalyticsSnapshot GetAnalytics(PlaytimeScope scope, PlaytimeRange range, DateTimeOffset? now = null, TimeZoneInfo? timeZone = null, IReadOnlyList<InstancePlaytimeSession>? activeSessions = null)
    {
        ...
        var calendarDays = range == PlaytimeRange.AllTime
            ? Math.Max(1, daily.Length == 0 ? 1 : DateOnly.FromDateTime(localNow.Date).DayNumber - daily[0].Date.DayNumber + 1)
        ...
        return new()
        {
            ...
            DailyBuckets = daily,
            HeatmapBuckets = heat.Select(x => new PlaytimeHeatmapBucket(x.Key.Item1, x.Key.Item2, x.Value)).OrderBy(x => x.Weekday).ThenBy(x => x.Hour).ToArray(),
    ...
    private PlaytimeHistoryEnvelope ReadEnvelope()
    {
        ...
        {
            _isReadOnly = true;
            _logger.LogWarning("Playtime history schema {SchemaVersion} is newer than supported schema {SupportedSchemaVersion}; leaving it unchanged.", envelope.SchemaVersion, SchemaVersion);
        }
        return envelope;
    }

    private IReadOnlyList<InstancePlaytimeSession> ReadHistory() => ReadEnvelope().Sessions;

    private static void SplitSession(InstancePlaytimeSession session, TimeZoneInfo zone, DateOnly firstDate, DateTimeOffset localNow, Dictionary<DateOnly, TimeSpan> days, Dictionary<(DayOfWeek, int), TimeSpan> hours)
    {
    ...
    private static (int Current, int Longest) CalculateStreaks(DateOnly[] dates, DateOnly today)
    {
        ...
        {
            if (dates[i].DayNumber == dates[i - 1].DayNumber + 1) streak++;
            else streak = 1;
            longest = Math.Max(longest, streak);
        }
        var last = dates[^1];
        var current = last == today || last == today.AddDays(-1) ? 1 : 0;
        for (var i = dates.Length - 2; current > 0 && i >= 0 && dates[i + 1].DayNumber == dates[i].DayNumber + 1; i--) current++;
