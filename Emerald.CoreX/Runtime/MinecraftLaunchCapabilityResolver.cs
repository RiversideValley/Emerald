...
public sealed record MinecraftLaunchCapabilities(
    bool CanLaunchServer,
    bool CanQuickPlayWorld,
    string? WorldLaunchUnavailableReason = null);
...
public sealed class MinecraftLaunchCapabilityResolver(ILogger<MinecraftLaunchCapabilityResolver> logger)
    : IMinecraftLaunchCapabilityResolver
{
    private readonly ConcurrentDictionary<string, MinecraftLaunchCapabilities> _cache =
        new(StringComparer.OrdinalIgnoreCase);

    public async Task<MinecraftLaunchCapabilities> ResolveAsync(Game game,
    ...
    private async Task<MinecraftLaunchCapabilities> InspectAsync(Game game, string versionId,
        CancellationToken cancellationToken)
    {
        try
        {
            var supportsQuickPlay = await InspectVersionChainAsync(game.Path.Versions, versionId,
                new HashSet<string>(StringComparer.OrdinalIgnoreCase), cancellationToken);
            return supportsQuickPlay
                ? new MinecraftLaunchCapabilities(true, true)
                : new MinecraftLaunchCapabilities(true, false,
                    "This Minecraft version does not expose singleplayer Quick Play.");
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or JsonException)
        {
            logger.LogWarning(ex, "Could not inspect launch capabilities for version {VersionId}.", versionId);
            return new MinecraftLaunchCapabilities(true, false,
                "The custom version metadata could not be read safely.");
        }
    }

    private static async Task<bool> InspectVersionChainAsync(string versionsPath, string versionId,
        HashSet<string> visited, CancellationToken cancellationToken)
    {
        if (!visited.Add(versionId) || visited.Count > 16)
        {
            return false;
        }
        ...
        if (ContainsQuickPlaySingleplayer(document.RootElement))
        {
            return true;
        }
