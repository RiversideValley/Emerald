...
public sealed class AuthlibInjectorService : IAuthlibInjectorService
{
        ...
        DownloadTimeouts? timeouts = null)
    {
        ...
        _baseDirectory = string.IsNullOrWhiteSpace(baseDirectory)
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Emerald",
                "authlib-injector")
            : baseDirectory;
    ...
    public async Task<AuthlibInjectorLaunchConfiguration> PrepareLaunchAsync(
    ...
    public async Task<IReadOnlyList<AuthlibInjectorVersion>> GetAvailableVersionsAsync(
        CancellationToken cancellationToken = default)
    {
        await _preparationGate.WaitAsync(cancellationToken).ConfigureAwait(false);
        try
        {
            ...
            try
            {
                index = await GetJsonWithRetriesAsync<ArtifactIndexDto>(
                        new Uri(_options.ArtifactApiRoot, "artifacts.json"), cancellationToken)
                                       ...
                                       !cancellationToken.IsCancellationRequested)
            {
                _logger.LogWarning(ex,
                    "Could not refresh the authlib-injector version catalog; trying the cached catalog.");
                index = await ReadJsonFileAsync<ArtifactIndexDto>(CatalogPath, cancellationToken).ConfigureAwait(false)
                        ?? throw new InvalidOperationException(
                            "The authlib-injector version catalog is unavailable and no cached catalog exists.", ex);
                ValidateIndex(index);
            }
    ...
    private Selection GetSelection()
    {
        ...
        {
            AuthlibInjectorVersionMode.Recommended => new Selection(_settings.VersionMode, _options.RecommendedVersion),
            AuthlibInjectorVersionMode.Latest => new Selection(_settings.VersionMode, null),
            AuthlibInjectorVersionMode.Custom when !string.IsNullOrWhiteSpace(_settings.CustomVersion)
                => new Selection(_settings.VersionMode, _settings.CustomVersion.Trim()),
            AuthlibInjectorVersionMode.Custom => throw new InvalidOperationException(
                "Choose an authlib-injector version in Advanced settings before launching with Ely.by."),
            _ => throw new InvalidOperationException(
                $"Unsupported authlib-injector version mode '{_settings.VersionMode}'.")
        };
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (selection.Mode == AuthlibInjectorVersionMode.Latest)
            {
                descriptor = ValidateDescriptor(await GetJsonWithRetriesAsync<ArtifactDescriptor>(
                        new Uri(_options.ArtifactApiRoot, "artifact/latest.json"), cancellationToken)
                    .ConfigureAwait(false));
            }
            else
            {
                var index = await GetJsonWithRetriesAsync<ArtifactIndexDto>(
                        new Uri(_options.ArtifactApiRoot, "artifacts.json"), cancellationToken)
                    .ConfigureAwait(false);
                ValidateIndex(index);
                await WriteJsonAtomicallyAsync(CatalogPath, index, cancellationToken).ConfigureAwait(false);
                ...
                {
                    throw new AuthlibSelectionException(
                        $"Authlib-injector version '{selection.Version}' is not present in the official catalog.");
                }

                var build = catalogEntry.BuildNumber;
                descriptor = ValidateDescriptor(await GetJsonWithRetriesAsync<ArtifactDescriptor>(
                        new Uri(_options.ArtifactApiRoot, $"artifact/{build}.json"), cancellationToken)
                    .ConfigureAwait(false));
                if (descriptor.BuildNumber != build ||
                    !string.Equals(descriptor.Version, selection.Version, StringComparison.Ordinal))
                {
                    throw new InvalidDataException(
                        "The official authlib-injector artifact descriptor did not match the selected catalog entry.");
                }
            }
        ...
        catch (Exception ex) when (ex is not OperationCanceledException || !cancellationToken.IsCancellationRequested)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The official authlib-injector artifact API is unavailable and no cached descriptor matches the selected version.",
                    ex);
            }
    ...
    private async Task<string> EnsureVerifiedJarAsync(ArtifactDescriptor descriptor,
        CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(jarPath))
        {
            ...
            {
                _logger.LogInformation("Verified cached authlib-injector build {Build} with SHA-256.",
                    descriptor.BuildNumber);
                return jarPath;
            }

            _logger.LogWarning(
                "Cached authlib-injector build {Build} failed SHA-256 validation; downloading a replacement.",
                descriptor.BuildNumber);
        }
        ...
        for (var attempt = 1; attempt <= Math.Max(1, _timeouts.Attempts); attempt++)
        {
            ...
            {
                _logger.LogInformation(
                    "Downloading authlib-injector {Version} build {Build}, attempt {Attempt}/{Attempts}.",
                ...
                {
                    throw new InvalidDataException(
                        $"Downloaded authlib-injector build {descriptor.BuildNumber} failed SHA-256 validation.");
                }

                File.Move(tempPath, jarPath, true);
                _logger.LogInformation(
                    "Verified authlib-injector build {Build} with SHA-256 and stored it in the cache.",
                    descriptor.BuildNumber);
                return jarPath;
            }
            catch (Exception ex) when (ex is not OperationCanceledException ||
        ...
        throw new InvalidOperationException(
            $"Could not download and verify authlib-injector {descriptor.Version} build {descriptor.BuildNumber}.",
            lastError);
    }
    ...
    private async Task<MetadataResult> ResolveMetadataAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var endpoint in new[] { _options.PrimaryMetadataEndpoint, _options.FallbackMetadataEndpoint })
        {
            try
            {
                ...
                {
                    _logger.LogInformation("Using Ely.by metadata fallback endpoint {Endpoint}.",
                        SafeEndpoint(result.ApiRoot));
                }
                                       ...
                                       !cancellationToken.IsCancellationRequested)
            {
                ...
                _logger.LogWarning("Ely.by metadata endpoint {Endpoint} failed: {Reason}.", SafeEndpoint(endpoint),
                    reason);
            }
        }

        throw new InvalidOperationException(
            "Ely.by launch metadata could not be obtained, so Minecraft was not started. " +
    ...
    private async Task<MetadataResult> FetchMetadataAsync(Uri initialEndpoint, CancellationToken cancellationToken)
    {
        ...
        for (var redirects = 0; redirects < 4; redirects++)
        {
            ...
            {
                throw new HttpRequestException($"HTTP {(int)response.StatusCode} ({response.StatusCode})", null,
                    response.StatusCode);
            }
            ...
            var bytes = await ReadLimitedAsync(response.Content, _options.MaximumMetadataBytes, cancellationToken)
                .ConfigureAwait(false);
    ...
    private async Task<T> GetJsonWithRetriesAsync<T>(Uri endpoint, CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= attempts; attempt++)
        {
                ...
                && IsRetryableArtifactFailure(ex))
            {
                ...
                {
                    _logger.LogWarning(ex,
                        "Authlib-injector artifact request attempt {Attempt}/{Attempts} failed; retrying.", attempt,
                        attempts);
                }
            }
        }
    ...
    private async Task<HttpResponseMessage> GetResponseAsync(Uri endpoint, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var response = await _httpClient.GetAsync(endpoint, HttpCompletionOption.ResponseHeadersRead, timeout.Token)
                .ConfigureAwait(false);
    ...
    private async Task DownloadFileAsync(Uri endpoint, string destination, CancellationToken cancellationToken)
    {
        ...
        await using var target =
            new FileStream(destination, FileMode.CreateNew, FileAccess.Write, FileShare.None, 81920, true);
    ...
    private async Task<byte[]> ReadLimitedAsync(HttpContent content, int maximumBytes,
        CancellationToken cancellationToken)
    {
    ...
    private static void ValidateMetadata(byte[] bytes)
    {
        try
        {
            ...
            if (root.ValueKind != JsonValueKind.Object
                ...
                || !meta.TryGetProperty("implementationName", out var implementation) ||
                implementation.ValueKind != JsonValueKind.String
                || string.IsNullOrWhiteSpace(implementation.GetString())
                || !root.TryGetProperty("skinDomains", out var skinDomains) ||
                skinDomains.ValueKind != JsonValueKind.Array
                || !root.TryGetProperty("signaturePublickey", out var publicKey) ||
    ...
    private async Task WriteDescriptorAsync(ArtifactDescriptor descriptor, AuthlibInjectorVersionMode mode,
        CancellationToken cancellationToken)
    {
        await WriteJsonAtomicallyAsync(DescriptorPath(descriptor.BuildNumber), descriptor, cancellationToken)
            .ConfigureAwait(false);
        if (mode == AuthlibInjectorVersionMode.Latest)
        {
            await WriteJsonAtomicallyAsync(LatestSelectionPath,
                    new LatestSelectionDto { BuildNumber = descriptor.BuildNumber }, cancellationToken)
                .ConfigureAwait(false);
        }
    }

    private async Task<ArtifactDescriptor?> FindCachedDescriptorAsync(Selection selection,
        CancellationToken cancellationToken)
    {
        if (selection.Mode == AuthlibInjectorVersionMode.Latest)
        {
            var latest = await ReadJsonFileAsync<LatestSelectionDto>(LatestSelectionPath, cancellationToken)
                .ConfigureAwait(false);
            return latest is { BuildNumber: > 0 }
                ? ValidateCached(
                    await ReadJsonFileAsync<ArtifactDescriptor>(DescriptorPath(latest.BuildNumber), cancellationToken)
                        .ConfigureAwait(false))
                : null;
        }
        ...
        {
            var descriptor = ValidateCached(await ReadJsonFileAsync<ArtifactDescriptor>(path, cancellationToken)
                .ConfigureAwait(false));
    ...
    private async Task WriteJsonAtomicallyAsync<T>(string path, T value, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await File.WriteAllTextAsync(tempPath, JsonSerializer.Serialize(value, _jsonOptions), cancellationToken)
                .ConfigureAwait(false);
    ...
    private static bool IsRetryableArtifactFailure(Exception exception)
    {
        return exception switch
        {
            ...
            {
                StatusCode: HttpStatusCode.RequestTimeout or HttpStatusCode.TooManyRequests
            } => true,
    ...
    {
        return endpoint.GetLeftPart(UriPartial.Path);
    }
    ...
    private string DescriptorPath(int build)
    {
        return Path.Combine(_baseDirectory, $"artifact-{build}.json");
    }
    ...
    private sealed class ArtifactIndexDto
    {
        ...
        [JsonPropertyName("artifacts")] public List<ArtifactIndexEntryDto> Artifacts { get; init; } = [];
    }
    ...
    private sealed class ArtifactIndexEntryDto
    {
        ...
        [JsonPropertyName("build_number")] public int BuildNumber { get; init; }
    }
    ...
    private sealed class ArtifactDescriptor
    {
        ...
        [JsonPropertyName("release_time")] public DateTimeOffset? ReleaseTime { get; init; }

        [JsonPropertyName("download_url")] public Uri DownloadUrl { get; init; } = null!;

        [JsonPropertyName("checksums")] public ArtifactChecksums Checksums { get; init; } = new();

        [JsonIgnore] public string Sha256 => Checksums.Sha256;
    }
