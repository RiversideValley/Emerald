...
public static class MinecraftServerAddressParser
{
    public static bool TryParse(string? input, out MinecraftServerAddress address, out string? error)
    {
        ...
        if (string.IsNullOrWhiteSpace(value)) { error = "Enter a server address."; return false; }
        if (value.Contains("://", StringComparison.Ordinal) || value.Any(char.IsWhiteSpace) || value.IndexOfAny(['/', '?', '#']) >= 0) { error = "Enter a host name or IP address without a URI scheme, path, or spaces."; return false; }
        ...
        if (value.StartsWith('['))
        {
            ...
            if (close < 2 || (close + 1 < value.Length && value[close + 1] != ':')) { error = "The bracketed IPv6 address is invalid."; return false; }
            host = value[1..close];
            if (close + 1 < value.Length && !TryPort(value[(close + 2)..], out port, out error)) return false;
            if (!IPAddress.TryParse(host, out var parsed) || parsed.AddressFamily != System.Net.Sockets.AddressFamily.InterNetworkV6) { error = "The bracketed address is not valid IPv6."; return false; }
        }
        else
        {
            ...
            if (colonCount > 1)
            {
                if (!IPAddress.TryParse(value, out var parsed) || parsed.AddressFamily != System.Net.Sockets.AddressFamily.InterNetworkV6) { error = "The IPv6 address is invalid."; return false; }
                host = value;
            }
    ...
    {
        error = null;
        if (!int.TryParse(value, out port) || port is < 1 or > 65535) { error = "The port must be between 1 and 65535."; return false; }
        return true;
    }
}
...
public sealed class SavedServerService(IBaseSettingsService settings, ILogger<SavedServerService> logger) : ISavedServerService
{
    ...
    public SavedServer Save(SavedServer server)
    {
        ...
        {
            var envelope = Read();
            var duplicate = envelope.Servers.FirstOrDefault(x => new MinecraftServerAddress(x.Host, x.Port).CanonicalKey == parsed.CanonicalKey && x.Id != server.Id);
            if (_isReadOnly) return server;
            if (duplicate != null)
            {
                duplicate.Name = string.IsNullOrWhiteSpace(server.Name) ? duplicate.Name : server.Name.Trim();
                Write(envelope); Changed?.Invoke(this, EventArgs.Empty); return duplicate;
            }
            var existing = envelope.Servers.FirstOrDefault(x => x.Id == server.Id);
            if (existing == null) envelope.Servers.Add(server);
            else Copy(server, existing);
            Write(envelope);
        }
    ...
    {
        lock (_gate) { var envelope = Read(); if (_isReadOnly) return; var server = envelope.Servers.FirstOrDefault(x => x.Id == id); if (server == null) return; server.LastLaunchedAt = at ?? DateTimeOffset.UtcNow; Write(envelope); }
        Changed?.Invoke(this, EventArgs.Empty);
    }
    ...
    private void Write(SavedServerEnvelope value) => settings.Set(SettingsKeys.SavedServers, value);
    private static void Copy(SavedServer source, SavedServer destination) { destination.Name = source.Name; destination.Host = source.Host; destination.Port = source.Port; destination.SourceKind = source.SourceKind; destination.SourceSlug = source.SourceSlug; destination.SourcePageUrl = source.SourcePageUrl; destination.IconUrl = source.IconUrl; destination.BannerUrl = source.BannerUrl; destination.LastLaunchedAt = source.LastLaunchedAt; }
}

public enum ServerDirectorySort { Votes, Players, Rating, Newest, Name }
...
public sealed record ServerDirectoryEntry(string Name, string Host, int Port, string? Slug, string? PageUrl, string? Motd, string? Version, int Players, int MaxPlayers, string? IconUrl, string? BannerUrl, IReadOnlyList<string> Tags, double? Rating, int Votes, bool Online)
{
...
public interface IServerDirectoryService { Task<ServerDirectoryPage> SearchAsync(ServerDirectoryQuery query, CancellationToken cancellationToken = default); }
...
public sealed class ServerDirectoryService(HttpClient httpClient, INetworkCapabilityService network, ILogger<ServerDirectoryService> logger) : IServerDirectoryService
{
    ...
    {
        var parameters = new Dictionary<string, string> { ["edition"] = "java", ["page"] = Math.Max(1, query.Page).ToString(), ["per"] = Math.Clamp(query.PageSize, 1, 50).ToString(), ["sort"] = SortName(query.Sort) };
        if (!string.IsNullOrWhiteSpace(query.Search)) parameters["q"] = query.Search.Trim();
        if (!string.IsNullOrWhiteSpace(query.Tag)) parameters["tag"] = query.Tag.Trim();
        if (!string.IsNullOrWhiteSpace(query.Version)) parameters["version"] = query.Version.Trim();
        ...
        lock (_gate) if (_cache.TryGetValue(url, out var hit) && DateTimeOffset.UtcNow - hit.At < TimeSpan.FromMinutes(1)) return hit.Page;
        ...
        try
        {
            using var response = await httpClient.GetAsync(url, deadline.Token); response.EnsureSuccessStatusCode();
            ...
            lock (_gate) _cache[url] = new(DateTimeOffset.UtcNow, page);
    ...
    private static ServerDirectoryPage ParseDirectory(JsonElement root, ServerDirectoryQuery query)
    {
        ...
        foreach (var x in array.EnumerateArray())
        {
            ...
            items.Add(new(Text(x, "name", "title") ?? host, host, port, Text(x, "slug"), Text(x, "url", "website"), Text(x, "motd", "tagline", "description"), Text(x, "version"), Int(players, "online") ?? Int(x, "online_players") ?? 0, Int(players, "max") ?? Int(x, "max_players", "maxPlayers") ?? 0, Text(x, "icon", "icon_url"), Text(x, "banner", "banner_url"), Strings(x, "tags"), Double(rating, "average") ?? Double(x, "rating"), Int(votes, "month") ?? Int(x, "votes") ?? 0, Bool(x, "online") ?? true));
        }
        return new(items, query.Page, query.PageSize, Int(root, "total", "count"));
    }
    private static JsonElement Find(JsonElement x, params string[] names) { foreach (var n in names) if (x.ValueKind == JsonValueKind.Object && x.TryGetProperty(n, out var v)) return v; return default; }
    private static string? Text(JsonElement x, params string[] names) { var v = Find(x, names); return v.ValueKind == JsonValueKind.String ? v.GetString() : null; }
    private static int? Int(JsonElement x, params string[] names) { var v = Find(x, names); return v.ValueKind == JsonValueKind.Number && v.TryGetInt32(out var n) ? n : null; }
    private static double? Double(JsonElement x, params string[] names) { var v = Find(x, names); return v.ValueKind == JsonValueKind.Number && v.TryGetDouble(out var n) ? n : null; }
    private static bool? Bool(JsonElement x, params string[] names) { var v = Find(x, names); return v.ValueKind is JsonValueKind.True or JsonValueKind.False ? v.GetBoolean() : null; }
    private static IReadOnlyList<string> Strings(JsonElement x, params string[] names) { var v = Find(x, names); return v.ValueKind == JsonValueKind.Array ? v.EnumerateArray().Where(y => y.ValueKind == JsonValueKind.String).Select(y => y.GetString()!).ToArray() : []; }
...
public sealed class ServerStatusService(HttpClient httpClient, INetworkCapabilityService network, ILogger<ServerStatusService> logger, IServerAddressClassifier? classifier = null) : IServerStatusService
{
    ...
    public async Task<ServerStatusSnapshot> GetStatusAsync(MinecraftServerAddress address, bool forceRefresh = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            using var response = await httpClient.SendAsync(request, deadline.Token); response.EnsureSuccessStatusCode();
            using var document = JsonDocument.Parse(await response.Content.ReadAsStreamAsync(deadline.Token));
    ...
    private static ServerStatusSnapshot ParseStatus(JsonElement x, MinecraftServerAddress requested)
    {
        ...
        var motd = x.TryGetProperty("motd", out var m) && m.TryGetProperty("clean", out var clean) ? string.Join(Environment.NewLine, clean.EnumerateArray().Select(y => y.GetString())) : null;
        return new(online ? ServerStatusState.Online : ServerStatusState.Offline, DateTimeOffset.UtcNow, requested,
            Text(x, "ip"), Int(x, "port"), Text(x, "version"), x.TryGetProperty("protocol", out var protocol) ? Int(protocol, "version") : null,
            Text(x, "software"), motd, Int(players, "online") ?? 0, Int(players, "max") ?? 0, Text(x, "map"), Bool(x, "eula_blocked"), Text(x, "icon"), Names(x, "plugins"), Names(x, "mods"));
    }
    private static string? Text(JsonElement x, string name) => x.ValueKind == JsonValueKind.Object && x.TryGetProperty(name, out var v) && v.ValueKind == JsonValueKind.String ? v.GetString() : null;
    private static int? Int(JsonElement x, string name) => x.ValueKind == JsonValueKind.Object && x.TryGetProperty(name, out var v) && v.TryGetInt32(out var n) ? n : null;
    private static bool? Bool(JsonElement x, string name) => x.ValueKind == JsonValueKind.Object && x.TryGetProperty(name, out var v) && v.ValueKind is JsonValueKind.True or JsonValueKind.False ? v.GetBoolean() : null;
    private static IReadOnlyList<string> Names(JsonElement x, string name) => x.ValueKind == JsonValueKind.Object && x.TryGetProperty(name, out var v) && v.ValueKind == JsonValueKind.Array ? v.EnumerateArray().Take(25).Select(y => y.ValueKind == JsonValueKind.Object ? Text(y, "name") : y.GetString()).Where(y => y != null).Cast<string>().ToArray() : [];
}
