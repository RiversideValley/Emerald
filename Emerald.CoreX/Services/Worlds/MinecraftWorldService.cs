...
public enum MinecraftWorldGameMode
{
    Survival,
    Creative,
    Adventure,
...
public partial class MinecraftWorld : ObservableObject
{
    ...
    public string SizeText =>
        SizeBytes is long size ? FormatSize(size) : SizeUnavailable ? "Unavailable" : "Calculating…";

    partial void OnSizeUnavailableChanged(bool value)
    {
...
public sealed class MinecraftWorldService(ILogger<MinecraftWorldService> logger, IUiDispatcher? dispatcher = null)
    : IMinecraftWorldService
{
    ...
    public async Task<IReadOnlyList<MinecraftWorld>> ScanAsync(Game game, CancellationToken cancellationToken = default)
    {
        ...
        return await Task.Run(() =>
        {
            ...
            return (IReadOnlyList<MinecraftWorld>)worlds.OrderByDescending(x => x.LastPlayed).ThenBy(x => x.DisplayName)
                .ToArray();
        }, cancellationToken);
    }
    ...
    public async Task<long?> CalculateSizeAsync(MinecraftWorld world, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            return await Task.Run(() =>
            {
                ...
                (dispatcher ?? new InlineUiDispatcher()).Invoke(() => world.SizeBytes = total);
                return (long?)total;
            }, cancellationToken);
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            logger.LogDebug(ex, "Could not calculate world size.");
            (dispatcher ?? new InlineUiDispatcher()).Invoke(() => world.SizeUnavailable = true);
            return null;
        }
        finally
        {
    ...
    private MinecraftWorld ParseWorld(DirectoryInfo directory, string primary, string backup, bool linked)
    {
        ...
        if (File.Exists(primary))
        {
            try
            {
                return ReadMetadata(directory, primary, false, linked);
            }
        ...
        if (File.Exists(backup))
        {
            try
            {
                return ReadMetadata(directory, backup, true, linked);
            }
            catch (Exception ex) when (IsMetadataFailure(ex))
            {
                primaryError ??= ex;
        ...
        {
            FolderName = directory.Name, FolderPath = directory.FullName, DisplayName = directory.Name,
            IconPath = ExistingIcon(directory), IsReadable = false, IsLinked = linked,
            Warning = "World metadata is unreadable."
        };
    }
    ...
    private static MinecraftWorld ReadMetadata(DirectoryInfo directory, string path, bool backup, bool linked)
    {
        ...
        {
            throw new InvalidDataException("World metadata exceeds the safe input limit.");
        }

        var file = new NbtFile(new NbtOptions
            { Flavor = NbtFlavor.Java, ValidateOnRead = true, MaxAllocation = MaxMetadataBytes });
        ...
        var seed = data.Get<NbtCompound>("WorldGenSettings")?.Get<NbtLong>("seed")?.Value ??
                   data.Get<NbtLong>("RandomSeed")?.Value;
        ...
        return new MinecraftWorld
        {
            ...
            IsReadable = true,
            IsLinked = linked,
            Warning = linked ? "This world folder is linked; size traversal is disabled." :
                backup ? "Recovered metadata from level.dat_old." : null
        };
    }

    private static MinecraftWorldGameMode ToMode(int? mode)
    {
        return mode switch
        {
            0 => MinecraftWorldGameMode.Survival, 1 => MinecraftWorldGameMode.Creative,
            2 => MinecraftWorldGameMode.Adventure, 3 => MinecraftWorldGameMode.Spectator,
            _ => MinecraftWorldGameMode.Unknown
        };
    }

    private static string? ExistingIcon(DirectoryInfo directory)
    {
        var path = Path.Combine(directory.FullName, "icon.png");
        return File.Exists(path) ? path : null;
    }

    private static bool IsContained(string parent, string candidate)
    {
        var root = Path.TrimEndingDirectorySeparator(parent) + Path.DirectorySeparatorChar;
    ...
    {
        return ex is IOException or UnauthorizedAccessException or InvalidDataException or NbtFormatException
