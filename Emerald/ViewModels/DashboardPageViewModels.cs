...
public sealed record HomeSelection(Game Game, MinecraftLaunchTargetKind Kind, CoreX.Services.Servers.SavedServer? Server = null,
    CoreX.Services.Worlds.MinecraftWorld? World = null, CoreX.Models.EAccount? Account = null, Guid? ProfileId = null);
...
{
    public static string Get(string key) => ("Refine" + key).Localize();
    public static string Format(string key, params object[] args) => string.Format(CultureInfo.CurrentCulture, Get(key), args);
    public static string Duration(TimeSpan value) => value.TotalHours >= 1 ? Format("HoursMinutes", (int)value.TotalHours, value.Minutes)
    ...
    {
        if (value == null) return Get("NeverPlayed");
        var local = value.Value.ToLocalTime(); var days = (DateTime.Today - local.Date).Days;
        return days == 0 ? Format("TodayAt", local.ToString("t")) : days == 1 ? Format("YesterdayAt", local.ToString("t")) : local.ToString("d");
    }
    public static string Glyph(MinecraftLaunchTargetKind kind) => kind switch { MinecraftLaunchTargetKind.World => "\uE909", MinecraftLaunchTargetKind.Server => "\uE968", _ => "\uE768" };
    public static string Target(MinecraftLaunchTargetKind kind) => Get(kind == MinecraftLaunchTargetKind.World ? "World" : kind == MinecraftLaunchTargetKind.Server ? "Server" : "MainMenu");
    ...
    public static IReadOnlyList<PlaytimeBarViewModel> Bars(IEnumerable<PlaytimeDayBucket> buckets, int days, double height)
    {
        ...
        var all = Enumerable.Range(0, days).Select(i => new PlaytimeDayBucket(start.AddDays(i), values.GetValueOrDefault(start.AddDays(i)))).ToArray();
        var max = Math.Max(1, all.Select(x => x.Duration.TotalSeconds).DefaultIfEmpty().Max());
        return all.Select(x => new PlaytimeBarViewModel(x.Date.ToString("ddd"), $"{x.Date:d} · {Duration(x.Duration)}", height * x.Duration.TotalSeconds / max)).ToArray();
    }
}
