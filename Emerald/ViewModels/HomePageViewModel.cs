...
public sealed class QuickProfileCardViewModel
{
    ...
    public QuickProfileCardViewModel(QuickProfile profile, QuickProfileValidation validation)
    {
        Profile = profile;
...
public partial class HomePageViewModel : ObservableObject
{
    ...
    private async Task LaunchAsync()
    {
        ...
        {
            LaunchMessage = "Choose an account before launching.";
            return;
        }

        if (!SelectedGame.CanLaunch)
        {
            LaunchMessage = SelectedGame.InstallationState == InstanceInstallationState.Ready
                ? "This instance is already active."
                : "Manage or repair this instance before launching.";
            return;
        }

        var target = await BuildTargetAsync();
        if (target == null)
        {
            return;
        }
        ...
        try
        {
            await _runtime.LaunchAsync(new GameLaunchRequest(SelectedGame, SelectedAccount, target, ActiveQuickProfileId));
        }
        finally
        {
            IsBusy = false;
    ...
    public async Task SelectProfileAsync(QuickProfile profile, bool launch)
    {
        ...
        {
            _notifications.Warning("Profile needs attention", string.Join(" ", validation.Messages));
            return;
        }
        ...
        _accounts.SetSelectedAccount(account);
        ...
        {
            SelectedDestination = "Server";
            SelectedServer = FavoriteServers.First(x => x.Id == profile.SavedServerId);
        }
        else if (profile.TargetKind == MinecraftLaunchTargetKind.World)
        {
            SelectedDestination = "World";
            SelectedWorld = RecentWorlds.First(x => x.FolderName == profile.WorldFolderName);
        }
        else
        {
            SelectedDestination = "Main menu";
        }
    ...
    private async Task<MinecraftLaunchTarget?> BuildTargetAsync()
    {
        ...
        if (SelectedDestination == "World")
        {
            ...
            {
                LaunchMessage = "Choose a world or browse local saves.";
                return null;
            }

            var capability = await _capabilities.ResolveAsync(SelectedGame!);
            if (!capability.CanQuickPlayWorld)
            {
                LaunchMessage = capability.WorldLaunchUnavailableReason;
                return null;
            }
    ...
    private async Task LoadWorldsAsync()
    {
        ...
        try
        {
            RecentWorlds.ReplaceWith((await _worlds.ScanAsync(SelectedGame)).Take(8));
            SelectedWorld ??= RecentWorlds.FirstOrDefault();
    ...
    private void RefreshAnalytics()
    {
        ...
        var scope = ShowAllEmeraldPlaytime || SelectedGame == null
        ...
        TotalPlaytimeText = FormatDuration(result.TotalPlaytime);
        WeekPlaytimeText = FormatDuration(result.CurrentWeekPlaytime);
        SessionCountText = $"{result.CompletedSessionCount} sessions";
        var maximum = Math.Max(1, result.DailyBuckets.Select(x => x.Duration.TotalMinutes).DefaultIfEmpty().Max());
        HomeDailyBars.ReplaceWith(result.DailyBuckets.TakeLast(7).Select(x =>
            new PlaytimeBarViewModel(
                x.Date.ToString("ddd"),
                FormatDuration(x.Duration),
                Math.Max(3, 34 * x.Duration.TotalMinutes / maximum))));
    }
    ...
    private async Task RefreshFavoriteStatusAsync()
    {
        ...
        if (favorite == null)
        {
            FavoriteServerStatus = "NoFavoriteServerYet".Localize();
            return;
        }
        ...
        {
            FavoriteServerStatus = string.Format("FavoriteServerUnavailableFormat".Localize(), favorite.Name);
        }
    }
