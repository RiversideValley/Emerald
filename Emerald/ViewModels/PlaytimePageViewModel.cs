...
public partial class PlaytimePageViewModel(Core core, IInstancePlaytimeService playtime, IGameRuntimeService runtime)
    : ObservableObject
{
    ...
    public IReadOnlyList<Choice<PlaytimeRange>> Ranges { get; } =
    [
        new(PlaytimeRange.SevenDays, DashboardText.Get("SevenDays")),
        new(PlaytimeRange.ThirtyDays, DashboardText.Get("ThirtyDays")),
        new(PlaytimeRange.NinetyDays, DashboardText.Get("NinetyDays")),
        new(PlaytimeRange.AllTime, DashboardText.Get("AllTime"))
    ];
    ...
    {
        playtime.HistoryChanged += HistoryChanged;
    }
    ...
    public void Initialize(PlaytimeNavigation? navigation = null)
    {
        ...
        Scopes.Clear();
        Scopes.Add(new Choice<PlaytimeScope>(PlaytimeScope.AllEmerald, DashboardText.Get("AllEmerald")));
        if (core.BasePath is { } path)
        {
            Scopes.Add(
                new Choice<PlaytimeScope>(PlaytimeScope.ForBase(path.BasePath), DashboardText.Get("CurrentBase")));
            foreach (var game in core.Games)
            {
                Scopes.Add(new Choice<PlaytimeScope>(PlaytimeScope.ForInstance(path.BasePath, game.InstanceId),
    ...
    partial void OnSelectedScopeChanged(Choice<PlaytimeScope>? value)
    {
        if (!_initializing)
        {
            _sessionPage = _rankingPage = 0;
            Refresh();
        }
    }

    partial void OnSelectedRangeChanged(Choice<PlaytimeRange>? value)
    {
        if (!_initializing)
        {
            _sessionPage = _rankingPage = 0;
    ...
    public void Refresh()
    {
        ...
        var range = SelectedRange?.Value ?? PlaytimeRange.ThirtyDays;
        ...
        UpdateItems(Kpis, [
            ...
            new PlaytimeKpiViewModel(DashboardText.Get("ActiveDays"), r.ActiveDayCount.ToString()),
            new PlaytimeKpiViewModel(DashboardText.Get("LongestSession"),
                DashboardText.Duration(r.LongestCompletedSession)),
            new PlaytimeKpiViewModel(DashboardText.Get("CurrentStreak"),
                DashboardText.Format("Days", r.CurrentStreakDays))
        ]);
        BuildChart(range);
        var heat = r.HeatmapBuckets.ToDictionary(x => (x.Weekday, x.Hour));
        var max = Math.Max(1, r.HeatmapBuckets.Select(x => x.Duration.TotalSeconds).DefaultIfEmpty().Max());
        ...
        BestDateDuration = r.MostPlayedDate is { } best
            ...
            : string.Empty;
        FavoriteDay = r.MostPlayedWeekday is { } day
            ? System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.GetDayName(day)
            : DashboardText.Get("NoPatterns");
        PeakHour = r.PeakLocalHour is int hour
            ? $"{hour:00}:00–{(hour + 1) % 24:00}:00"
            : DashboardText.Get("NoPatterns");
        PeakDuration = r.PeakLocalHour is int peak
            ? DashboardText.Duration(
                TimeSpan.FromTicks(r.HeatmapBuckets.Where(x => x.Hour == peak).Sum(x => x.Duration.Ticks)))
            : string.Empty;
        _sessionPage = Math.Min(_sessionPage, Math.Max(0, (r.Sessions.Count - 1) / 10));
        _rankingPage = Math.Min(_rankingPage, Math.Max(0, (r.InstanceRanking.Count - 1) / 5));
        UpdatePages();
    ...
    private void BuildChart(PlaytimeRange range)
    {
        ...
        var first = range switch
        {
            PlaytimeRange.SevenDays => today.AddDays(-6), PlaytimeRange.ThirtyDays => today.AddDays(-29),
            PlaytimeRange.NinetyDays => today.AddDays(-89),
            _ => _result.DailyBuckets.FirstOrDefault()?.Date ?? today
        };
        var length = today.DayNumber - first.DayNumber + 1;
        var mode = range == PlaytimeRange.NinetyDays || (range == PlaytimeRange.AllTime && length > 31)
            ? length > 366 ? 2 : 1
            : 0;
        ...
        var values = Enumerable.Range(0, length).Select(i => first.AddDays(i)).GroupBy(d =>
                mode == 2 ? new DateOnly(d.Year, d.Month, 1) :
                mode == 1 ? first.AddDays((d.DayNumber - first.DayNumber) / 7 * 7) : d)
            .Select(g => new
            {
                Start = g.Key, End = g.Max(),
                Duration = TimeSpan.FromTicks(g.Sum(d => lookup.GetValueOrDefault(d).Ticks))
            }).ToArray();
    ...
    {
        return count == 0
            ? DashboardText.Get("NoSessions")
            : DashboardText.Format("PageCount", page * size + 1, Math.Min(count, (page + 1) * size), count);
    }

    public void ChangeSessionPage(int offset)
    {
        _sessionPage = Math.Clamp(_sessionPage + offset, 0, Math.Max(0, (_result.Sessions.Count - 1) / 10));
        UpdatePages();
    }

    public void ChangeRankingPage(int offset)
    {
        _rankingPage = Math.Clamp(_rankingPage + offset, 0, Math.Max(0, (_result.InstanceRanking.Count - 1) / 5));
        UpdatePages();
    }
        ...
        foreach (var name in new[]
                 {
                     ...
                     nameof(CanPreviousRanking), nameof(CanNextRanking), nameof(SessionPageText),
                     nameof(RankingPageText)
                 })
        {
