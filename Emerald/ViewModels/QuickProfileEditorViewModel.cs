...
public partial class QuickProfileEditorViewModel : ObservableObject
{
 ...
 public QuickProfileEditorViewModel(HomePageViewModel home, IAccountService accounts, IQuickProfileService profiles, IMinecraftWorldService worlds, QuickProfile? original)
 {
  ...
  Icon = Icons.FirstOrDefault(x => x.Key == original?.GlyphKey) ?? Icons.First(x => x.Key == (IsWorld ? "World" : IsServer ? "Server" : "Play"));
  IconKind = original?.IconKind ?? QuickProfileIconKind.Glyph;
  BlockIconFileName = original?.BlockIconFileName;
  if (IconKind == QuickProfileIconKind.Block && string.IsNullOrWhiteSpace(BlockIconFileName)) IconKind = QuickProfileIconKind.Glyph;
 ...
 partial void OnIconChanged(QuickProfileGlyphOption? value) => Update();
 partial void OnIconKindChanged(QuickProfileIconKind value) { OnPropertyChanged(nameof(IsGlyphIcon)); OnPropertyChanged(nameof(IsBlockIcon)); Update(); }
 partial void OnBlockIconFileNameChanged(string? value) => Update();
 ...
 public QuickProfile CreateProfile() => new()
 {
  ...
  Name = Name.Trim(), GlyphKey = Icon?.Key ?? "Play", IconKind = this.IconKind, BlockIconFileName = IsBlockIcon ? this.BlockIconFileName : null, AccentArgb = Accent?.Value ?? 0xFF107C10,
 ...
 private void Update()
 {
  ...
  Error = string.IsNullOrWhiteSpace(Name) ? DashboardText.Get("EnterName") : IsBlockIcon && string.IsNullOrWhiteSpace(BlockIconFileName) ? DashboardText.Get("ChooseIcon") : SelectedGame == null ? DashboardText.Get("ChooseInstance")
