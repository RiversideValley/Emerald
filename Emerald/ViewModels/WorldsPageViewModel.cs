...
public sealed class WorldRowViewModel(MinecraftWorld world)
{
    ...
    public string Metadata => string.Join(" · ",
        new[]
        {
            DashboardText.Get(World.GameMode.ToString()), World.MinecraftVersion,
            DashboardText.Relative(World.LastPlayed)
...
public partial class WorldsPageViewModel(
    Core core,
    IAccountService accounts,
    IMinecraftWorldService worlds,
    IMinecraftLaunchCapabilityResolver capabilities,
    IGameRuntimeService runtime) : ObservableObject
{
    ...
    public IReadOnlyList<Choice<int>> Sorts { get; } =
    [
        new(0, DashboardText.Get("LastPlayed")), new(1, DashboardText.Get("Name")), new(2, DashboardText.Get("Size")),
        new(3, DashboardText.Get("GameMode"))
    ];
    ...
    partial void OnSelectedSortChanged(Choice<int>? value)
    {
        if (!_initializing)
        {
            if (value?.Value == 2)
    ...
    partial void OnLaunchLimitationChanged(string? value)
    {
        OnPropertyChanged(nameof(HasLimitation));
    }
    ...
    {
        _load?.Cancel();
        var load = _load = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        var game = SelectedGame;
        ErrorMessage = null;
        LaunchLimitation = null;
        if (game == null)
        {
            Worlds.Clear();
            ApplyFilter();
        ...
        try
        {
            var result = await worlds.ScanAsync(game, load.Token);
            var caps = await capabilities.ResolveAsync(game, load.Token);
            if (load.IsCancellationRequested || SelectedGame != game)
            {
                return;
            }
            ...
            {
                await Task.WhenAll(result.Select(x => worlds.CalculateSizeAsync(x, load.Token)));
            }
    ...
    public async Task<bool> LaunchAsync(MinecraftWorld world)
    {
        ...
        if (game == null || !game.CanLaunch || !world.IsReadable)
        {
            ErrorMessage = DashboardText.Get("WorldUnavailable");
            return false;
        }

        try
        {
            var caps = await capabilities.ResolveAsync(game);
            ...
            await runtime.LaunchAsync(new GameLaunchRequest(game, account,
    ...
    private void ApplyFilter()
    {
        ...
        {
            1 => query.OrderBy(x => x.DisplayName), 2 => query.OrderByDescending(x => x.SizeBytes),
            3 => query.OrderBy(x => x.GameMode), _ => query.OrderByDescending(x => x.LastPlayed)
        };
        FilteredWorlds.ReplaceWith(query.Select(x => new WorldRowViewModel(x)));
        OnPropertyChanged(nameof(IsEmpty));
