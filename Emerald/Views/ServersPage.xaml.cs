...
public sealed partial class ServersPage : Page
{
    ...
    private async void Play_Row(object? sender, EventArgs e)
    {
        if (sender is ServerRow { Model: { } row })
        {
    ...
    {
        if (sender is not ServerRow { Model: { } row })
        {
            return;
        }

        if (row.IsFavorite && await new ContentDialog
            {
                XamlRoot = XamlRoot, Title = DashboardText.Get("RemoveFavorite"),
                Content = DashboardText.Get("RemoveFavoriteHint"), PrimaryButtonText = DashboardText.Get("Remove"),
                CloseButtonText = DashboardText.Get("Cancel"), DefaultButton = ContentDialogButton.Close
    ...
    {
        if (sender is not ServerRow { Model: { } row } anchor)
        {
            return;
        }
        ...
        {
            var item = new MenuFlyoutItem { Text = DashboardText.Get(key) };
        ...
        if (row.Saved != null)
        {
            Add("RefreshStatus", async () => await ViewModel.RefreshStatusAsync(true));
        }

        if (row.Saved?.SourceKind == SavedServerSourceKind.Custom)
        {
            Add("Edit", async () => await ShowCustomAsync(row.Saved));
        }
        ...
        {
            Add("SourcePage", async () => await Windows.System.Launcher.LaunchUriAsync(uri));
        }
    ...
    private void Discover_Click(object sender, RoutedEventArgs e)
    {
        ...
        ((Microsoft.UI.Xaml.Controls.Primitives.ToggleButton)sender).IsChecked = true;
    }
    ...
    {
        if (ViewModel.IsDiscover)
        {
            await ViewModel.SearchAsync();
        }
        else
        {
            await ViewModel.RefreshStatusAsync(true);
        }
    }
    ...
    private async Task ShowCustomAsync(SavedServer? original)
    {
        ...
        {
            Header = DashboardText.Get("Address"), Text = original?.Address ?? string.Empty,
        ...
        {
            XamlRoot = XamlRoot, Title = DashboardText.Get(original == null ? "AddServer" : "EditServer"),
            Content = panel, PrimaryButtonText = DashboardText.Get("Save"),
            CloseButtonText = DashboardText.Get("Cancel"), DefaultButton = ContentDialogButton.Primary
        };
        dialog.PrimaryButtonClick += (_, args) =>
        {
            if (!MinecraftServerAddressParser.TryParse(address.Text, out var _, out var message))
            {
                args.Cancel = true;
                error.Text = message;
            }
        ...
        {
            ViewModel.SaveCustom(name.Text, address.Text, original);
    ...
    private void Layout_SizeChanged(object sender, SizeChangedEventArgs e)
    {
        ...
        Grid.SetColumn(InstancePicker, compact ? 0 : 3);
        Grid.SetColumnSpan(InstancePicker, compact ? 3 : 1);
