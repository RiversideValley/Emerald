...
{
    private const string NightlyArtifactsFallbackUrl =
        "https://github.com/RiversideValley/Emerald/actions/workflows/ci.yml?query=branch%3Amain";
    ...
    public ChannelOption SelectedUpdateChannel
    {
        get
        {
            ...
            return _availableChannels.FirstOrDefault(option => option.Channel == selectedChannel) ??
    ...
    private async Task CheckForUpdatesAsync()
    {
        ...
        var operation = _notifications.Create("CheckForUpdates".Localize(), "CheckingUpdates".Localize(),
            ...
            switch (result.Status)
            {
                case AppUpdateStatus.UpdateAvailable:
                {
                    ...
                    if (response == LocalMessageBoxResults.CustomResult1)
                    {
                        ...
                        {
                            await MessageBox.Show("Error".Localize(),
                                installResult.Message ?? "CheckForUpdates".Localize(), LocalMessageBoxButtons.Ok);
                            _notifications.Complete(operation.Id, false,
                                installResult.Message ?? "CheckForUpdates".Localize(), installResult.Error);
                            return;
                        }
                    }
                ...
                case AppUpdateStatus.UpToDate:
                    await MessageBox.Show("NoUpdatesAvailable".Localize(), "NoUpdates".Localize(),
                ...
                case AppUpdateStatus.LocalBuildIsNewer:
                    await MessageBox.Show("DowngradeAvailable".Localize(), "DowngradeDescription".Localize(),
                        LocalMessageBoxButtons.Ok);
                ...
                case AppUpdateStatus.ManualDownloadRequired:
                {
                    ...
                    if (response == LocalMessageBoxResults.CustomResult1)
                    {
                        ...
                        if (!opened)
                        {
                            await MessageBox.Show("Error".Localize(), "CouldNotOpenNightlyArtifacts".Localize(),
                                LocalMessageBoxButtons.Ok);
                        }
                    }
                ...
                default:
                    await MessageBox.Show("Error".Localize(), result.ErrorMessage ?? "CheckForUpdates".Localize(),
                        LocalMessageBoxButtons.Ok);
