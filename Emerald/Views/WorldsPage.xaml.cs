...
public sealed partial class WorldsPage : Page
{
    ...
    {
        if (sender is FrameworkElement { Tag: MinecraftWorld world })
        {
            FileManager.Reveal(world.FolderPath);
        }
    }
    ...
    private async void Launch_Click(object sender, RoutedEventArgs e)
    {
        if (sender is FrameworkElement { Tag: MinecraftWorld world })
        {
            await ViewModel.LaunchAsync(world);
        }
    }

    private async void Details_Click(object sender, RoutedEventArgs e)
    {
        if (sender is not FrameworkElement { Tag: MinecraftWorld world })
        {
            return;
        }

        var seed = world.Seed?.ToString() ?? "Unavailable";
        var copy = new Button
        {
            Content = "Copy seed",
            IsEnabled = world.Seed.HasValue,
        ...
        {
            Content = "Select for Home",
            HorizontalAlignment = HorizontalAlignment.Left
        };
        select.Click += (_, _) =>
        {
            var home = Ioc.Default.GetRequiredService<HomePageViewModel>();
            home.SelectedGame = ViewModel.SelectedGame;
            home.SelectedDestination = "World";
            home.SelectedWorld = world;
            Frame.Navigate(typeof(HomePage));
        };
